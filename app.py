...
def get_result_from_database(sql_query):
    server_name = "sql-server-ol5eg6w3zlsm4.database.windows.net" #os.environ.get("server_name")
    database_name = "DiseaseRegistriesDB" #os.environ.get("database_name")
    username = 
    password = 
...
async def handle_query():
    ...
    asyncClient = AsyncOpenAI(base_url="",
                          api_key="Not needed by Compass API, only for passing the validation of the SDK",
                          default_headers={"api-key": ""})

    kernel.add_text_completion_service("dv", OpenAIChatCompletion(ai_model_id="gpt-4",async_client=asyncClient))
